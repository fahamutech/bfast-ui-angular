 ...
 */
export const styleCreateComponent = async function (project, module, style = {
    name: '',
    body: ''
}, styles = [], error = null) {
    return `
            ...
            <div>
                <div>
                    <div class="d-flex lex-row" style="margin-bottom: 8px">
                        <h3><a href="/project/${project}/modules/${module}/resources/styles" >${style.name} style</a> update</h3>
                        <span style="flex: 1 1 auto"></span>
        <!--                        <button class="btn btn-sm btn-primary">Save Service</button>-->
                    </div>
                    <input class="form-control" readonly value="${style.name}" name="name" placeholder="enter service name" type="text">
                </div>
                <hr>
...
<!--                      <span class="visually-hidden">Loading...</span>-->
                ...
                <div>
                    ${codeEditorComponent('styleCode', style.body, 'scss', 'updateStyleButton', null)}
                </div>
                <script>
                    document.getElementById('updateStyleButton').onclick = ev => {
                        ...
                        fetch('/project/${project}/modules/${module}/resources/styles/${style.name}', {
                            method: 'POST',
                            headers: {
