export const stateInjectionTableComponent = async function (project, module, state, injections = [], services = []) {
    return `
        ...
        <div class="shadow">
            <table class="table table-hover">
              ...
              <tbody>
               ${getTableContents(project, module, state, injections)}
              </tbody>
            </table>
            ${await addInjectionModal(project, module, state, services)}
        </div>
    `
}

function getTableContents(project, module, state, injections = []) {
    let row = '';
        ...
        row += `<tr style="cursor: pointer">
                  ...
                  <td>
                    <div class="d-flex flex-row">
                        <form method="post" action="/project/${project}/modules/${module}/resources/states/${state}/injections/${injection.service}.service.ts/delete">
...
async function addInjectionModal(project, module, state, services) {
    function allOtherServices() {
        let otherServices = ''
        for (const injection of services) {
            otherServices += `
            <div style="margin-bottom: 5px">
                <form method="post" action="/project/${project}/modules/${module}/resources/states/${state}/injections/${injection}">
