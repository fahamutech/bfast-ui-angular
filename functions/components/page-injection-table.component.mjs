export const pageInjectionTableComponent = async function (project, module, page, injections = [], services = []) {
    return `
        ...
        <div class="shadow">
            <table class="table table-hover">
              ...
              <tbody>
               ${getTableContents(project, module, page, injections)}
              </tbody>
            </table>
            ${await addInjectionModal(project, module, page, services)}
        </div>
    `
}

function getTableContents(project, module, page, injections = []) {
    let row = '';
        ...
        row += `<tr style="cursor: pointer">
                  ...
                  <td>
                    <div class="d-flex flex-row">
                        <form method="post" action="/project/${project}/modules/${module}/resources/pages/${page}/injections/${injection.state}.state.ts/delete">
...
async function addInjectionModal(project, module, page, services) {
    function allOtherServices() {
        let otherServices = ''
        for (const injection of services) {
            otherServices += `
            <div style="margin-bottom: 5px">
                <form method="post" action="/project/${project}/modules/${module}/resources/pages/${page}/injections/${injection}">
